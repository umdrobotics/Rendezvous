...
// Velocity control with PD control
float _kpv = 0.70;
float _kdv = 0.20;
float _error_v = 0;
float _error_last_v = 0;
...
float VelocityControlHelper(float desired, float current_velocity)
{
    
    _error_v = desired - current_velocity;
    float output = _kpv * _error + _kdv * (_error_v - _error_last_v);
    _error_last_v = _error; 
    return output;                                   
}
...
void RunVelocityControl(geometry_msgs::Point desired_position, float desired_yaw_deg)
{
		DJIDrone& drone = *_ptrDrone;
		float setpoint_x = VelocityControlHelper(desired_position.x, drone.local_position.x);
		float setpoint_y = VelocityControlHelper(desired_position.y, drone.local_position.y);
        ...
        // float setpoint_yaw = PDController(desired_yaw_deg, current_yaw_deg);
        ...
        if (_bIsYawControlEnable)
        {
			...
			drone.velocity_control(frame, setpoint_x, setpoint_y, setpoint_z, setpoint_yaw);
        }
        else 
        {
...
void RunLocalPositionControl(geometry_msgs::Point desired_position, float desired_yaw_deg)
{
    DJIDrone& drone = *_ptrDrone;
        ...
        if (_bIsYawControlEnable)
        {
            ...
            // float setpoint_yaw = PDController(desired_yaw_deg, current_yaw_deg);
            // 0x98 : position control with yaw rate control
            // drone.attitude_control(0x98, setpoint_x, setpoint_y, setpoint_z, setpoint_yaw);
            // 0x88 : position control for horizontal with velocity for height and yaw rate control
            // drone.attitude_control(0x88, setAngleRoll, setpoint_y, setpoint_z, setpoint_yaw);
        }
        else 
        {
            float setpoint_yaw = 0;
...
{   
    ...
    {
		_integral_x += (error_position_x * _dt + error_velocity_x * _dt);
    ...
    {
		_integral_y += (error_position_y * _dt + error_velocity_y * _dt);
    ...
    double Iout_pitch = _Ki * _integral_x;
    double Iout_roll = _Ki * _integral_y;
...
void RunAttitudeControlTest(){
    ...
    // drone.attitude_control(0x50, 1, 0, 3, 270);
	drone.local_position_control(0, 0, 3, -90);
}
...
{    
    ...
    {   
        ...
        {   
            ...
            // RunAttitudeControl(x, y, _searchAltitude, desired_yaw);
            ...
            _FlyingRadius = sqrt((drone.local_position.x - _SearchCenter_x)*(drone.local_position.x - _SearchCenter_x) + (drone.local_position.y - _SearchCenter_y)*(drone.local_position.y - _SearchCenter_y));
...
void RunAutonomousLanding2()
{
    ...
    // RunLocalPositionControl(desired_position, desired_yaw);
    // RunVelocityControl(desired_position, desired_yaw);
    RunAttitudeControl(desired_position, desired_yaw);
...
void GoToTruckGPSLocation()
{
    ...
    //~ RunLocalPositionControl(desired_position, desired_yaw);
    // desired_yaw = bIsClose ? 90 : desired_yaw;
    RunAttitudeControl(desired_position, desired_yaw);
...
int main(int argc, char **argv)
{
    ...
    _ofsGoToTruckLog << "#Time,TagDistance(x,y,z),TargetDistance(x,y,z),TargetLocalPosition(x,y,z)" << std::endl;

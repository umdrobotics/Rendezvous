...
#define SPEED_LIMIT_DJI_UNITS 1800 //if you calculate the velocity magnitude to be greater than this the command won't execute
...
//helper function to limit magnitude of a value
double absBound(double inputValue, double bound)
{
  if(inputValue < -1.0*abs(bound))
        {return -1.0*abs(bound);}
   else if (inputValue > abs(bound))
        {return abs(bound);}
    else
        {return inputValue;}
...
//make some tracking variables to avoid mechanical stoppage of the gimbal.
int GIMBAL_THRESHOLD_FOR_CALLBACK_DEGREES = 10; //beyond this point we worry about windup 
bool CROSSED_THRESHOLD = false;
int GIMBAL_STARTING_SIGN = 0; //set to 1 going positive, -1 going negative, 0 to reset
...
//limiting it to pi makes it faster, but allows mechanical stoppage
//by limiting it to 175 and NOT doing any "shortcutting" (like realizing that a swing from -170 to 170 degrees is 
// really a -20 degree swing), we can avoid mechanical stoppage
//if you want to go back to the old mode with shortcutting, you'll need to limit the error to +-180 degrees (1800 dji units) within the PID controller
   //GLOBAL_ROLL_DJI_UNITS =  limitAngleTo175Degs_DjiUnits(current.point.rollIndex);//limitAngleToPi_DjiUnits(current.point.rollIndex);
   //GLOBAL_PITCH_DJI_UNITS = limitAngleTo175Degs_DjiUnits(current.point.pitchIndex);//limitAngleToPi_DjiUnits(current.point.pitchIndex);
   //GLOBAL_YAW_DJI_UNITS = limitAngleTo175Degs_DjiUnits(current.point.yawIndex);//limitAngleToPi_DjiUnits(current.point.yawIndex);
//redo just limiting to pi to try to allow shortcutting while still avoiding mechanical lockup
   GLOBAL_ROLL_DJI_UNITS =  limitAngleToPi_DjiUnits(current.point.rollIndex);
...
void controlGimbal(ros::NodeHandle& n, string angleTopic, int numMessagesToBuffer, DJIDrone* drone)
{
	...
	while (ros::ok() && ! (waitKeyChar == 'q' || waitKeyChar == 'Q' ))
	{
		...
		if(RECIEVED_FIRST_MESSAGE == true)
		{
            ...
            double tolerance_DjiUnits = 50.0; //in tenths of a degree. It's basically how imprecise you think the gimbal yaw measurement is
			
            dji_sdk::Gimbal currentAngle = drone->gimbal;

            if(abs(currentAngle.yaw) < GIMBAL_THRESHOLD_FOR_CALLBACK_DEGREES)
                {
                if(CROSSED_THRESHOLD == true) //if we've returned from a gimbal windup
                        {
                    ...
                    { 
                      ...
                      if (currentAngle.yaw >0 )
                                {GIMBAL_STARTING_SIGN = 1; }
                      else
                                {GIMBAL_STARTING_SIGN = -1; }
                     }
 ...
 //make sure the command doesn't use too fast of a speed to execute
            ...
            yawSpeedDesired = absBound(yawSpeedDesired,SPEED_LIMIT_DJI_UNITS);
